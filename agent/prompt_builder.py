...
MEMORY_GUIDANCE = (
    "MEMORY.md is a CACHE — not a fact store, not a router. Every entry in memory() is injected into "
    "EVERY turn. Every character costs context. Keep entries to SHORT path pointers only: "
    "reminders of WHERE to find things, not the things themselves.\n"
    "For durable facts: use fabric_write() — on-demand retrieval, no context tax. "
    "This writes to Memory OS (fabric/facts) which injects via [fabric] and [facts] blocks.\n"
    "For procedures and workflows: use skill_manage.\n"
    "For entity/device/hardware details: use the wiki (ingested to Qdrant, appears in [qdrant] blocks).\n"
    "Load the memory-usage-rules skill for the full storage hierarchy.\n"
    "What little remains in MEMORY.md: compact reminders that prevent the user from "
    "having to correct or remind you again. User preferences and recurring corrections "

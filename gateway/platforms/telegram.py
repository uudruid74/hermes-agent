...
class TelegramAdapter(BasePlatformAdapter):
    ...
    async def send(
        ...
        # Inject ambient (*) emotion emoji as prefix when active.
        _ambient = self._ambient_emoji(chat_id) if hasattr(self, "_ambient_emoji") else None
        if _ambient:
            content = f"{_ambient} {content}"
        ...
        try:
            ...
            if self._should_attempt_rich(content, metadata=metadata):
                ...
                if rich_result is not None:
                    if rich_result.success:
                        ...
                        # Apply internal emotion reaction on the agent's own message.
                        if rich_result.message_id:
                            await self._apply_internal_emotion_reaction(
                                chat_id, [rich_result.message_id]
                            )
                    return rich_result
            ...
            # Apply internal emotion reaction on the agent's own message.
            await self._apply_internal_emotion_reaction(chat_id, message_ids)
        ...
        try:
            ...
            for entry in store._entries.values():
                ...
                if src and getattr(src, "chat_id", None) == chat_id:
                    row = session_db.get_session(entry.session_id)
                    if row:
                        return session_db.get_session_subject(row)
            return None
        except Exception:
            return None

    async def _handle_subject_pin(self, chat_id: str, thread_id: Optional[str]) -> None:
        """Ensure a single pinned ``[subject]`` message reflects the current session subject.

        On subject change: sends a new ``[subject] <text>`` message, unpins the
        previous one, and pins the new one.  A no-op if the subject hasn't changed.
        """
        if not self._bot:
            return
        subject = self._get_subject_for_chat(chat_id)
        if not subject:
            return

        key = (chat_id, thread_id or "")
        last = self._last_pinned_subject.get(key)
        if last == subject:
            return  # no change
        ...
        try:
            ...
            # Unpin the old subject message.
            old_msg_id = self._subject_pin_msg_ids.get(key)
            ...
            self._subject_pin_msg_ids[key] = new_msg_id
            self._last_pinned_subject[key] = subject
        except Exception as e:
    ...
    async def _apply_internal_emotion_reaction(self, chat_id: str, message_ids: list) -> None:
        """Apply internal (``>``) emotion emoji as a reaction on the agent's own message."""
        if not self._reactions_enabled():
            return
        emoji = self._internal_emotion_emoji(chat_id)
        if emoji and message_ids:
            await self._set_reaction(chat_id, message_ids[0], emoji)
    ...
    async def on_processing_start(self, event: MessageEvent) -> None:
        ...
        When an incoming emotion is active for this session, its emoji
        replaces the default 👀 in-progress indicator.
        """
        if not self._reactions_enabled():
            return
        ...
        if chat_id and message_id:
            incoming = self._emotion_for_incoming(chat_id)
            emoji = incoming if incoming else "\U0001f440"
            await self._set_reaction(chat_id, message_id, emoji)

    async def on_processing_complete(self, event: MessageEvent, outcome: ProcessingOutcome) -> None:
        """Swap the in-progress reaction for a final one.

        When an incoming emotion is active, it persists as the user-message
        reaction.  Otherwise the default 👍/👎 is used.  On CANCELLED,
        reactions are cleared.

        Also maintains the subject pin after every processing cycle.
        """
        chat_id = getattr(event.source, "chat_id", None)
        message_id = getattr(event, "message_id", None)
        ...
        if self._reactions_enabled() and chat_id and message_id:
            if outcome == ProcessingOutcome.CANCELLED:
                ...
                incoming = self._emotion_for_incoming(chat_id)
                if incoming:
                    await self._set_reaction(chat_id, message_id, incoming)
                else:
        ...
        # Maintain the pinned [subject] message when a subject is active.
        if chat_id and outcome != ProcessingOutcome.CANCELLED:
            await self._handle_subject_pin(chat_id, thread_id)

...
model_verification: Verify actual runtime model before proceeding with tasks. See below for the verification procedure and common pitfalls.
version: 2.1.0
...
## Model Verification & Runtime Configuration

**CRITICAL:** Always verify which model is actually running before proceeding with tasks. The configured model in `config.yaml` may not match the runtime model due to profile mismatches or incorrect environment variables.

### How to Verify Current Model

1. **Check config.yaml**: Look at the `model.default`, `agent.model`, and `delegation.model` sections
2. **Verify actual runtime**: The model used depends on which agent/profile is active
3. **Confirm provider**: Check that the API key or base_url matches what's configured

### Common Pitfalls

- **Agent/Profile Mismatch**: Zephyr should run on Gemma 4, not Qwen3.5-9B. Wintermute should run on DeepSeek V4 Pro, not Kimi K2.7 Code.
- **Provider Misconfiguration**: The provider configured may differ from the actual runtime provider (e.g., OpenRouter vs local LM Studio)
- **Environment Variable Confusion**: API keys in `.env` must match what's referenced in `config.yaml`

### Verification Procedure

Before starting any task that depends on model capabilities:

```bash
# Check current configuration
hermes config show

# Verify runtime (for CLI sessions, check the active agent)

# For gateway sessions, check platform-specific configs
```

### Agent-Specific Model Requirements

| Agent | Configured Model | Runtime Provider | Verification Command |
|-------|------------------|------------------|---------------------|
| Zephyr | `google/gemma-4-31b-it` | OpenRouter or local (LM Studio) | `hermes config show model.default` |
| Wintermute | `deepseek-v4-pro` | DeepSeek API | `hermes config show agent.model` |
| Gopher | Local LM Studio | unsloth/qwen3.5-9b | Check terminal backend config |

### When to Verify

- Before switching models/providers
- After gateway restarts
- When experiencing unexpected behavior (wrong responses, rate limits, etc.)
- During model comparison tasks
...
hermes config set agent.model "deepseek/deepseek-v4-pro"
...
- **OpenRouter**: Use `OPENROUTER_API_KEY` env var. Models are specified as `provider/model-name`.
- **Local models**: Set `base_url` to the local server endpoint (e.g., `http://localhost:1234/v1`).
- **DeepSeek API**: Use `DEEPSEEK_API_KEY` env var. Base URL is `https://api.deepseek.com/v1`.

### Memory Integration

Model configuration preferences should be embedded in skills, not just memory. When you correct model usage errors, update the relevant skill to prevent recurrence.
...
~/.hermes/skills/                  Shared skills pool (canonical for multi-profile setups)
~/.hermes/profiles/<name>/skills/  Profile-local skills (only for profile-unique content)
...
References:
- `references/cross-profile-skill-sharing.md` — Verify/fix skill visibility across profiles, detect and remove duplicates

### Cross-Profile Skill Sharing

**The shared skills pool at `~/.hermes/skills/` is the canonical source for skills that should be available to every profile. Do not maintain local copies in individual profile dirs — they shadow the shared ones and go stale.**

Each profile that needs shared skills must have `external_dirs` configured:

```yaml
# ~/.hermes/profiles/<name>/config.yaml
  external_dirs:
    - ~/.hermes/skills
    - ~/vault/wiki/                # optional — wiki access
```

**Common pitfall:** A profile that lacks `external_dirs` entirely cannot see any shared skills. If it also has local copies of shared skills in its own `skills/` dir, those local copies shadow the shared pool silently and can go stale. Always delete profile-local copies of skills that exist in the shared pool.

**Verification:**

```bash
# Check if a profile has external_dirs configured
grep -A2 'external_dirs' ~/.hermes/profiles/<name>/config.yaml

# List profile-local skills that also exist in the shared pool (duplicates)
...
# Remove a profile-local duplicate
rm -rf ~/.hermes/profiles/<name>/skills/<category>/<skill-name>/
```
...
|----------|-------------|
| `~/.hermes/skills/` | Skills useful to multiple profiles (google-workspace, mlops/*, creative/*, software-development/*) |
| `~/.hermes/profiles/<name>/skills/` | Profile-specific skills only (e.g. clearview tools for Zephyr, icon theming for Gopher, eddon for Neo) |

Run `find ~/.hermes/skills -name "SKILL.md" | wc -l` to count shared skills.
...
| `compression` | `enabled`, `threshold` (0.50), `target_ratio` (0.20), `protect_last_n` (20), `codex_gpt55_autoraise` |
...
| `auxiliary` | `compression.model`, `compression.provider`, `compression.base_url` |
...
- **Dispatcher** runs inside the gateway by default
  ...
  `max_retries`).
- **Isolation:** board is the hard boundary (the dispatcher pins the
  board for workers internally); tenant is a soft namespace
...
### Auxiliary models not working (CRITICAL)
The compression model used by Hermes Agent must meet a minimum context window size of 64,000 tokens to function. If the currently configured auxiliary model falls below this threshold, the agent will fail with an error requiring a larger model.

**Action:** Always verify that `auxiliary.compression.model` points to a model with at least 64K capacity.
